...
const TrendingEmblaSlider = (props: PropType) => {
  ...
  const fetchTrendingItemsByDay = async () => {
    if (timeFrame == 'day') {
      await axiosInstance.get(`trending/movie/day?language=en-US&page=1&api_key=${api_key}`)
        .then((response) => {
      ...
      await axiosInstance.get(`trending/movie/week?language=en-US&page=1&api_key=${api_key}`)
        .then((response) => {
    ...
    <section className='trending-section-container container'>
      ...
      <div className="embla__viewport" ref={emblaRef}>
        <div className="embla__container">
          {trendingData?.map((movie: TrendingMovieData, index) => (
